...
const MOCK = {
  "aurora-trails": {
    ...
    desc: "Experience northern lights, glacier walks, and guided night safaris in Iceland and Norway.",
    img: "https://images.unsplash.com/photo-1500534314209-a25ddb2bd429?auto=format&fit=crop&w=1200&q=80",
...
export default function Experience() {
  ...
  return (
    <main className="min-h-screen bg-[#010414] text-white py-12 px-6">
      <div className="max-w-5xl mx-auto bg-black/40 rounded-2xl p-6 shadow-lg">
        <button
          onClick={() => navigate(-1)}
          className="mb-4 px-3 py-2 rounded-md border border-white/10 text-sm"
        >
        ...
        <h1 className="text-3xl font-bold">{data.title}</h1>
        <p className="mt-4 text-gray-300">{data.desc}</p>

        <div className="mt-6 flex gap-3">
          <button
            onClick={() => navigate(`/destination/${data.destination}`)}
            className="px-5 py-3 rounded-full bg-gradient-to-r from-blue-500 to-purple-600 text-white"
          >
            View Destination
          </button>
