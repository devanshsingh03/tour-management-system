...
export default function AIAssistant() {
  ...
  const [messages, setMessages] = useState([
    {
      sender: "ai",
      text:
        "Hi! I'm TourX Assistant. I can help you find tours, destinations like Manali or Goa, book trips, explore categories, and navigate our website. What would you like to know?",
    },
  ]);
  const [input, setInput] = useState("");
  const [loading, setLoading] = useState(false);
  ...
  const startListening = () => {
    ...
    recognition.onerror = (event) => {
      // Common & harmless browser error
      if (event.error === "network") {
        console.warn("Speech recognition network error (ignored)");
        setListening(false);
        return;
      }
  ...
  const getAIResponse = (userMessage) => {
    ...
    const destinations = {
      manali: {
        route: "/tours",
        info:
          "Manali is a beautiful hill station in the Himalayas. Let me take you to Manali tours!",
      },
      goa: {
        route: "/destination/goa-beaches",
        info: "Goa offers beautiful beaches and nightlife. Let me show you!",
      },
      bali: {
        route: "/destination/bali",
        info: "Bali is a tropical paradise. Let me show you Bali!",
      },
      paris: {
        route: "/destination/paris",
        info: "Paris, the city of love! Let me take you there!",
      },
      dubai: {
        route: "/destination/dubai",
        info: "Dubai offers luxury and futuristic attractions!",
      },
      ladakh: {
        route: "/destination/ladakh",
        info: "Ladakh has breathtaking mountains and adventures!",
      },
    };
    ...
    if (message.includes("tour")) {
      return {
        text:
          "Let me show you all available tours! You can filter by category, price, or destination.",
        action: () => setTimeout(() => navigate("/tours"), 1500),
      };
    }
    ...
    if (message.includes("book")) {
      return {
        text:
          "To book a tour, browse tours, select one, and click Book Now. Let me take you there!",
        action: () => setTimeout(() => navigate("/tours"), 1500),
      };
    }
    ...
    if (message.includes("signup") || message.includes("register")) {
      return {
        text: "Let me take you to the signup page!",
        action: () => setTimeout(() => navigate("/signup"), 1500),
      };
    }
    ...
    if (message.includes("contact")) {
      return {
        text: "You can reach us through our Contact page!",
        action: () => setTimeout(() => navigate("/contact"), 1500),
      };
    }
    ...
    return {
      text:
        "I can help you find tours, destinations, book trips, and navigate the site. Try asking: 'Show Manali tours' or 'Take me to dashboard'.",
      action: null,
    };
  };
      ...
      {open && (
        ...
        >
          <div className="p-4 flex justify-between items-center bg-white/10 border-b border-white/20">
            ...
            <FiX
              onClick={() => setOpen(false)}
              className="text-black text-2xl cursor-pointer hover:text-red-600"
            />
          </div>
          ...
          <div className="p-3 bg-white/20 border-t border-white/30 flex items-center gap-2">
            ...
            <FiSend
              onClick={sendMessage}
              className="text-xl cursor-pointer text-black hover:text-blue-700"
            />
          </div>
        </div>
      )}
